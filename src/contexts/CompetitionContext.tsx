...
export function CompetitionProvider({ children }: { children: React.ReactNode }) {
  ...
  // ✅ Load competitions from Supabase (DO NOT CHANGE)
  useEffect(() => {
    (async () => {
  ...
  // ✅ Load scout_group_templates from Supabase (DO NOT CHANGE)
  useEffect(() => {
    (async () => {
  ...
  // ✅ Load scout_groups for selected competition (DO NOT CHANGE)
  useEffect(() => {
    if (!selectedId) return;
  ...
  // ✅ NEW: Load stations for selected competition
  useEffect(() => {
    if (!selectedId) return;
    ...
    (async () => {
      const { data, error } = await (supabase as any)
        .from("stations")
        .select("id,name,description,max_score,leader_email,allowed_sections,created_at,competition_id")
        .eq("competition_id", selectedId)
        .order("name", { ascending: true });
  ...
  // Competition management (DB + local state)
  ...
  const deleteCompetition = useCallback(async (id: string) => {
    // OBS: du kan behöva rensa child-tabeller i DB via FK/cascade eller manuellt
  ...
  const addPatrol = useCallback((patrol: Omit<Patrol, "id" | "createdAt">) => {
    updateCurrentCompetition((comp) => ({
      ...comp,
      patrols: [...comp.patrols, { ...patrol, id: Math.random().toString(36).slice(2), createdAt: new Date().toISOString() }],
    }));
  }, [updateCurrentCompetition]);
  ...
  // Scout group actions (DB) – oförändrat
  ...
  // Templates actions (DB) – oförändrat
  ...
  const deleteScoutGroupTemplate = useCallback(async (id: string) => {
    const { error } = await (supabase as any).from("scout_group_templates").delete().eq("id", id);
    if (error) {
  ...
  const setScore = useCallback((patrolId: string, stationId: string, score: number) => {
    updateCurrentCompetition((comp) => {
      ...
      const newScore: Score = {
        id: existingIndex >= 0 ? comp.scores[existingIndex].id : Math.random().toString(36).slice(2),
        patrolId,
        stationId,
        score,
  ...
  const getPatrolsWithScores = useCallback((section?: ScoutSection): PatrolWithScore[] => {
    ...
    patrolsWithScores.forEach((patrol, index) => { patrol.rank = index + 1; });
