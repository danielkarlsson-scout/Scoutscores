...
export default function Admin() {
  ...
  const toggleScorerRole = async (user: UserWithRoles) => {
    ...
    try {
      if (user.isScorer) {
        // Remove scorer role and all permissions
        await queryTable('user_roles')
          .delete()
          .eq('user_id', user.userId)
          .eq('role', 'scorer');
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Detta kommer att ta bort användaren "{user.displayName || user.email}" 
                              och all tillhörande data. Denna åtgärd går inte att ångra.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
